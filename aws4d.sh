...
function stop_instance {
    if [ "$#" != "1" ]; then
        echo "usage: stop_instance instance_tag_name" >&2
        return 1
    fi
...
function connect_to_instance {
    if [ "$#" != "1" ]; then
        echo "usage: connect_to_instance instance_tag_name" >&2
        return 1
    fi
    ...
    local dns_name=$(get_instance_public_dns_name ${name_tag}) \
        || { echo "Error: Could not get instance PublicDnsName" >&2; return 1; }
    update_security_group_my_ip "${name_tag}" \
        || { echo "Error: Could not update security group my ip" >&2; return 1; }
